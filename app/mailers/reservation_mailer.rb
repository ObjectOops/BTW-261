class ReservationMailer
  def self.confirmation(reservation)
    ...
    body    = <<~TEXT
      ...
      Additional users: #{reservation.additional_netids.present? ? "#{reservation.additional_netids}" : ''}
      Note: #{reservation.comment.present? ? "#{reservation.comment}" : ''}
